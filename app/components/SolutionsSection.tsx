        ...
        <div className="flex flex-wrap justify-center gap-x-8 gap-y-4 mb-16 border-b border-gray-200 pb-1">
          {TABS.map((tab) => (
            ...
            >
              ...
              {activeTab === tab.id && (
                <span className="absolute bottom-[-2px] left-0 w-full h-[4px] bg-[#CC0000] rounded-full" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {SOLUTIONS_DATA[activeTab as keyof typeof SOLUTIONS_DATA]?.map((item, index) => (
            <div key={index} className="relative group">
              <Card className="h-full border border-gray-100 shadow-sm group-hover:shadow-lg group-hover:border-[#CC0000] transition-all duration-300 pt-8 pb-12 bg-gray-50/50">
                <CardContent className="flex flex-col items-center text-center px-4">
                  {/* Icon Area */}
                  <div className="mb-6 text-gray-400 group-hover:text-[#CC0000] transition-colors duration-300 transform group-hover:scale-110">
                    {item.icon}
                  </div>
              ...
              <div className="absolute -bottom-5 left-0 right-0 flex justify-center z-10">
                <Button
                  size="icon"
                  className="rounded-full w-10 h-10 bg-[#CC0000] hover:bg-[#0F0F0F] text-white shadow-md transition-all duration-300 group-hover:scale-110"
                >
